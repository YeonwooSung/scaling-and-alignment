...
import vetrl.utils.hdfs_io as hdfs_io
...
from vetrl.utils.fsdp_utils import get_fsdp_wrap_policy, offload_fsdp_grad, init_fn, get_init_weight_context_manager
from vetrl.utils.fsdp_utils import offload_fsdp_optimizer, offload_fsdp_param_and_grad, load_fsdp_optimizer, \
    load_fsdp_param_and_grad
from vetrl.utils.import_utils import import_external_libs
from vetrl.utils.model import compute_position_id_with_mask
from vetrl.utils.flops_counter import FlopsCounter
from vetrl.workers.sharding_manager.fsdp_ulysses import FSDPUlyssesShardingManager
...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(self,
        ...
        if use_remove_padding:
            from vetrl.models.registry import check_model_support_rmpad
            check_model_support_rmpad(actor_model_config.model_type)

        if use_remove_padding and self.ulysses_sequence_parallel_size > 1:
            from vetrl.models.transformers.monkey_patch import apply_monkey_patch
    ...
    def _build_rollout(self):
        ...
        if self.config.rollout.name == 'hf':
            from vetrl.workers.rollout import HFRollout
            from vetrl.workers.sharding_manager import BaseShardingManager
            rollout = HFRollout(module=self.actor_module_fsdp, config=self.config.rollout)
            rollout_sharding_manager = BaseShardingManager()
            # TODO: a sharding manager that do nothing?
        elif self.config.rollout.name == 'vllm':
            from vetrl.workers.rollout.vllm_rollout import vLLMRollout
            from vetrl.workers.sharding_manager import FSDPVLLMShardingManager
    ...
    def init_model(self):
        from vetrl.workers.actor import DataParallelPPOActor
...
class CriticWorker(Worker):
    ...
    def _build_critic_model_optimizer(self, config):
        ...
        from vetrl.utils.model import LambdaLayer, print_model_size, squeeze
        from vetrl.utils.torch_dtypes import PrecisionType
        from torch.distributed.fsdp import FullyShardedDataParallel as FSDP, ShardingStrategy, MixedPrecision
        from torch import optim
    ...
    def init_model(self):
        ...
        from vetrl.workers.critic import DataParallelPPOCritic
