...
import torch
from vetrl.utils.reward_score import gsm8k, math, multiply, countdown
...
def main_task(config):
    ...
    if config.actor_rollout_ref.actor.strategy == 'fsdp':
        ...
        assert config.actor_rollout_ref.actor.strategy == config.critic.strategy
        from vetrl.workers.megatron_workers import ActorRolloutRefWorker, CriticWorker
    ...
    if config.reward_model.enable:
        if config.reward_model.strategy == 'fsdp':
            from vetrl.workers.fsdp_workers import RewardModelWorker
        elif config.reward_model.strategy == 'megatron':
            from vetrl.workers.megatron_workers import RewardModelWorker
        else:
            raise NotImplementedError
