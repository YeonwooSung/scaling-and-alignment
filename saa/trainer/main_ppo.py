...
def main_task(config, compute_score=None):
    ...
    if config.actor_rollout_ref.actor.strategy == 'fsdp':
        ...
        assert config.actor_rollout_ref.actor.strategy == config.critic.strategy
        from saa.workers.megatron_workers import ActorRolloutRefWorker, CriticWorker
    ...
    if config.reward_model.enable:
        if config.reward_model.strategy == 'fsdp':
            from saa.workers.fsdp_workers import RewardModelWorker
        elif config.reward_model.strategy == 'megatron':
            from saa.workers.megatron_workers import RewardModelWorker
        else:
            raise NotImplementedError
    ...
    if reward_manager_name == 'naive':
        from saa.workers.reward_manager import NaiveRewardManager
        reward_manager_cls = NaiveRewardManager
    elif reward_manager_name == 'prime':
        from saa.workers.reward_manager import PrimeRewardManager
