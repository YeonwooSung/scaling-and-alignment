...
from saa.utils.fsdp_utils import get_fsdp_wrap_policy, offload_fsdp_grad, init_fn, get_init_weight_context_manager
from saa.utils.fsdp_utils import offload_fsdp_optimizer, offload_fsdp_param_and_grad, load_fsdp_optimizer, \
    load_fsdp_param_and_grad
from saa.utils.import_utils import import_external_libs
from saa.utils.model import compute_position_id_with_mask
from saa.utils.flops_counter import FlopsCounter
from saa.utils.checkpoint.fsdp_checkpoint_manager import FSDPCheckpointManager
from saa.workers.sharding_manager.fsdp_ulysses import FSDPUlyssesShardingManager
...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(self,
                               ...
                               use_liger=False,
                               role='actor'):
        from saa.utils.model import print_model_size, update_model_config, get_generation_config
        from saa.utils.torch_dtypes import PrecisionType
        ...
        if use_remove_padding:
            from saa.models.registry import check_model_support_rmpad
            check_model_support_rmpad(actor_model_config.model_type)

        if use_remove_padding and self.ulysses_sequence_parallel_size > 1:
            from saa.models.transformers.monkey_patch import apply_monkey_patch
        ...
        if role == 'actor':
            from saa.utils.torch_functional import get_constant_schedule_with_warmup
    ...
    def _build_rollout(self):
        ...
        if self.config.rollout.name == 'hf':
            from saa.workers.rollout import HFRollout
            from saa.workers.sharding_manager import BaseShardingManager
            rollout = HFRollout(module=self.actor_module_fsdp, config=self.config.rollout)
            rollout_sharding_manager = BaseShardingManager()
            # TODO: a sharding manager that do nothing?
        elif self.config.rollout.name == 'vllm':
            from saa.workers.rollout.vllm_rollout import vLLMRollout, vllm_mode
            from saa.workers.sharding_manager import FSDPVLLMShardingManager
    ...
    def init_model(self):
        from saa.workers.actor import DataParallelPPOActor
...
class CriticWorker(Worker):
    ...
    def _build_critic_model_optimizer(self, config):
        ...
        from saa.utils.model import LambdaLayer, print_model_size, squeeze
        from saa.utils.torch_dtypes import PrecisionType
    ...
    def init_model(self):
        ...
        from saa.workers.critic import DataParallelPPOCritic
...
class RewardModelWorker(Worker):
    ...
    def compute_rm_score(self, data: DataProto):
        import itertools
        from saa.utils.seqlen_balancing import rearrange_micro_batches, get_reverse_idx
        data = data.to('cuda')
